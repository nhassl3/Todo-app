...
const TodosList = props => {
  ...
  const retrieveTodos = () => {
	...
	const deleteTodo = todoId => {
		...
		.catch(e => {
			console.error("Deleting", e.toString())
		})
	}
	...
	const completeTodo = todoId => {
		...
		.catch(e => {
			console.error("Complete Todo", e.toString())
		})
	}
							...
							<Card key={todo.id} className='mb-3'>
								<Card.Body>
									<div className={`${todo.completed ? "text-decoration-line-through": ""}`}>
										...
										<Card.Text className='mb-2'>Date created: {moment(todo.created).format("Do MMMM YYYY")}</Card.Text>
									</div>
									<>
										{!todo.completed ? <Link
										to={{
